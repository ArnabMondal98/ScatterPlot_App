...
Features local file picker, Z-score transformations, and comprehensive statistics.

Usage:
1. Run in VS Code with Python extension
2. Use interactive file picker to select Excel files
3. Choose variables and transformations
...
class ExcelScatterAnalyzer:
    """Excel Scatter Plot Analyzer with Z-Score Transformation"""
    ...
    def create_scatter_plot(self, x_var, y_var, color_var=None, 
        ...
        # Prepare data
        if color_var:
            plot_df = self.df[[x_var, y_var, color_var]].dropna()
        else:
            plot_df = self.df[[x_var, y_var]].dropna()
            ...
            for i, category in enumerate(categories):
                ...
                if np.any(cat_normal):
                    fig.add_trace(go.Scatter(
                        ...
                        hovertemplate=f'<b>{x_label}</b>: %{{x:.3f}}<br>' +
                                    ...
                                    f'<b>{color_var}</b>: {category}<extra></extra>'
                    ))
                ...
                if show_outliers and np.any(cat_outliers):
                    fig.add_trace(go.Scatter(
                        ...
                        hovertemplate=f'<b>{x_label}</b>: %{{x:.3f}}<br>' +
                                    f'<b>{y_label}</b>: %{{y:.3f}}<br>' +
                                    f'<b>{color_var}</b>: {category}<br>' +
                                    '<b>Status</b>: Outlier<extra></extra>'
                    ))
        else:
            # Single color
    ...
    def export_results(self, x_var, y_var, color_var=None):
        ...
        try:
            # Prepare export data
            if color_var:
                export_df = self.df[[x_var, y_var, color_var]].copy()
            else:
                export_df = self.df[[x_var, y_var]].copy()
    ...
    def run_interactive_analysis(self):
        ...
        print("=" * 60)
        print("Features: Z-score transformation, outlier detection, statistics")
        print()
        ...
        x_var, y_var, color_var = self.select_variables()
        ...
        if export_choice:
            self.export_results(x_var, y_var, color_var)
        
        print(f"\n✅ Analysis complete! 🎉")

# Convenience functions for VS Code interactive use
def quick_scatter(file_path, x_col, y_col, color_col=None, z_x=False, z_y=False):
    """Quick scatter plot function for VS Code"""
    analyzer = ExcelScatterAnalyzer()
    if analyzer.load_excel(file_path):
        fig = analyzer.create_scatter_plot(x_col, y_col, color_col, z_x, z_y)
...
def create_sample_excel():
    ...
    print("   • Height_cm vs Weight_kg (with Category coloring)")
    print("   • Education_years vs Income_USD (log-transformed)")
    print("   • Age_years vs Score (with Z-score transformation)")
